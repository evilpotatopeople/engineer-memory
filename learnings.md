...
- **stub 測試全綠不等於對；改 pipeline 後用最小真實店跑一次、3 秒、每次都做** — A-2 落地時 31 個 unittest ＋ /bin/bash 3.2 四情境模擬全綠，4 分鐘後用 aqua_hk（日均 0.6 單）跑真的 `sync_orders.py --auto-merge --no-cohort`，立刻抓到我把「restate 縮窗」當 partial 訊號的設計錯誤：fetch 完整、只是窗尾那天沒單 → exit 3 誤報、稀疏店每兩天中一次。stub 的 fetcher 與 CSV 都是我自己設計的、沒有「完整 fetch 但最後一天沒單」這種真實形狀。WIP docstring 還寫著「8 店每日 ≥ 9 單、實務上不會發生」——09-07 角落水族上車後假設就失效了、沒人回頭改。下次：任何 sync／pipeline 改動，測試綠了之後**一定**拿最小、最稀疏的真實店跑一次同參數（冪等、無害）；「實務上不會發生」這種寫進 code 的假設要標日期與前提、新品牌上車時 grep 一遍。
...
- **全 fragment 的 app 沒有「下一輪 rerun 會清掉」這回事** — 一次性訊息放 st.empty 想等下輪消失，但分頁互動只重跑 @st.fragment、外層元素永遠留著（登入成功訊息掛了一整天）。一次性 UI 要自己會消失：st.toast、或純 CSS 動畫淡出＋`:has()` 把外層容器 display:none；驗證時注意瀏覽器面板隱藏＝visibilityState hidden、CSS 動畫不會跑，要用 Web Animations API finish() 快轉看終態。
