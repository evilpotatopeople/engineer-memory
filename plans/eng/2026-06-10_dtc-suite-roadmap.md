...
## 推進清單 v3（2026-06-11 晚、價值排序版；含事中復盤兩層機制；v2 作廢）

排序邏輯：時效性（618 窗口內過期的最先）→ 復盤就緒（6/19 前讓復盤跑在強韌讀取上）→ 復盤解鎖件（資料翻倍/雙跑）→ 下輪規劃前的估算升級（ladyn 7 月底 summer 檔）→ 系統化→自動化。

1. [x] **事中 Tier1** ✅ 2026-06-12：`--record` + cron 10:45（user 授權後裝）+ 白名單；618 D1 六檔全自動記錄（hm_hk 🚀+25%、hm_tw 🔻-37%、lm_tw 🔻-17%、lm_hk 🔻-10%、ladyn 倒數）
2. [x] **事中 Tier2** ✅ 2026-06-12：`midflight_deep.py` 中場結構切片（11_中場切片_D{n}.xlsx）；**首航即歸因 TW 落後：兩品牌同款結構——新客營收占比僅假設一半（hm_tw 16.8% vs p50 30.8%、lm_tw 10.1% vs 23.1%）、老客回流大致正常**；D2-D3 續追驗證
3. **[6/19 前] P3b-A2+A3**：comparator sheet05 字串比對、report 寄倉表位置讀取 → sidecar 退役（復盤的 YoY/整合就跑在強韌讀取上）
4. **[6/19-20] 618 復盤（舊尺）→ 緊接 P4-3 校準表 v1**（pre+actual 配對 6→10 檔、資料翻倍趁熱建）
5. **[6/23-27] P3b-B 規則切換 + 復盤雙跑驗收 → P3 收官**（cohort configs 吃寄倉唯一源、hm_tw 首次寄倉 cohort、dashboard views 收斂）
6. **[6 月底-7 月初] P4 估算三連**：P4-0 語義標註+區間改名 → P4-1 三方法並排 → P4-2 shrinkage（趕在 ladyn summer 規劃前）
7. **[7 月] P5.5 系統化三件套**（2026-06-11 系統 vs 流程討論定）：campaign 生命週期狀態機 → `dtc` 門面指令 → 結果入庫 campaign_metrics（+`dtc health`）
   - 設計註記（2026-06-12 user 提問驗證）：目前 midflight 自動掃描綁「有 pre_estimate」＝「活動存在」是 planner 副作用、沒跑 planner 的活動隱形。狀態機後：登記活動＝明確動作（planned）、進檔期自動 live、midflight 自動掛上；無估算降級成純軌跡＋同 tag 歷史曲線、不隱形
8. **[7 月中] P6 自動化層 v1**：排程器（讀狀態機）+ 通知通道＝例外管理
9. **[7-8 月] 收尾帶**：P4-4 品類親和度、P4.5 案型工作流自動推進、P5-1 座標+信心、P5-2 透支、P5-3 ROAS+Meta API
10. **[8 月、雙11 前] P5-5 增量歸因**

## 推進清單 v2（已被 v3 取代、留存歷史）
